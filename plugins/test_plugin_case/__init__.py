...
class RichPrintCasePlugin(BasePlugin):
    """Rich/ANSI 打印与诊断插件（Win10 兼容性排查）"""
    ...
    def __init__(self, plugin_id, config_manager=None):
        ...
        self._entry_function_name = "diagnose_win10"
    ...
    def description(self):
        return "用于测试 ANSI 与 rich 彩色、表格/JSON/进度条显示，以及输出链路诊断"

    def run(self):
        # 运行诊断代码
        try:
            from test_plugin_case.rich_print_case import diagnose_win10
        except Exception:
            # 兼容放在 plugins/test_plugin_case 下的情况
            from plugins.test_plugin_case.rich_print_case import diagnose_win10
        try:
            # 输出标题
            self.log_output("开始执行 Win10 显示诊断…")
        except Exception:
            pass
        try:
            diagnose_win10()
